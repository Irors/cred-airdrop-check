...
async def request_(address: str, number: int, params):
    ...
    async with aiohttp.ClientSession() as session:
       ...
       try:
            response = await session.get(f'https://backend.townesquare.xyz/activity/point/{address}/WI3FA')
            token_1 = await get_response(response)
            await excel_write(address=address, quantity=token_1['totalPoint']-50, number=number)
